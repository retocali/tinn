...
// Gets largest index of float array
int xtgetLargestIndex(const float* arr, const int size) {
    float max = 0;
    int index = 0;
    for(int i = 0; i < size; i++) {
        if(arr[i]>max){
...
void xtprint(const float* arr, const int size) {
    ...
    printf("\n|");
    int index = xtgetLargestIndex(arr, size);
    for(int i = 0; i < size; i++) {
        if(i==index){
            printf("  X  |");
        } else {
...
// Prints image that is size x size
void xtprintImg(const float* img, const int size) {
    int index;
